...
def home(request):
    ...
    return render(request, 'home.html', {'first_news': first_news, 'three_news': three_news})
...
def category(request,id):
    ...
    return render(request, 'category-news.html', {'all_news':news, 'category':category})
def all_region(request):
    regs = Region.objects.all()
    return render(request, 'region.html', {'regs':regs})
def region(request,id):
    region = Region.objects.get(id=id)
    regs = News.objects.filter(region=region)
    return render(request, 'region-news.html', {'all-news':regs, 'region':region})
